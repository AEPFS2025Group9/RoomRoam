...
class Booking:
    ...
    def __init__(self, booking_id: int, guest_id: int, room_id: int, check_in_date: date, check_out_date: date, guest_count: int = 1):
        if booking_id is not None and (not isinstance(booking_id, int) or booking_id < 0):
            raise ValueError("Booking ID must be a positive integer or None")
    ...
    def __str__(self):
        return f"Booking[{self.__booking_id}] Guest {self.__guest_id}, Room {self.__room_id}, {self.__check_in_date} → {self.__check_out_date} ({self.__guest_count} guests)"
